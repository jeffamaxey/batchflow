...
def _get_method_by_alias(alias, module, tf_distributions=None):
    ...
    if fullname is None:
        raise ValueError(
            f"Distribution {alias} has no implementaion in module {module}"
        )

    # check that the randomizer is implemented in corresponding module
    if not hasattr(rnd_submodules[module], fullname):
        raise ValueError(
            f"Distribution {fullname} has no implementaion in module {module}"
        )
        ...
        while cumulated < size:
            ...
            if ctr > self.max_iters:
                if self.sample_anyways:
                    warnings.warn(
                        f"Already took {self.max_iters} number of iteration to make a sample. Yet, `sample_anyways`is set to true, so going on. Kill the process manually if needed."
                    )
                else:
                    raise ValueError(
                        f"The number of iterations needed to obtain the sample exceeds {self.max_iters}.Stopping the process."
                    )
